const fs = require('fs');
const readline = require('readline');

const logPath = "C:\\Users\\sudha\\.gemini\\antigravity-ide\\brain\\6b2987cc-c5b5-46ec-89d7-0a73830e78d0\\.system_generated\\logs\\transcript.jsonl";

if (!fs.existsSync(logPath)) {
  console.log(`Log path not found: ${logPath}`);
  process.exit(1);
}

const outStream = fs.createWriteStream('all_tool_calls.txt');
const rl = readline.createInterface({
  input: fs.createReadStream(logPath),
  crlfDelay: Infinity
});

rl.on('line', (line) => {
  try {
    const obj = JSON.parse(line);
    if (obj.tool_calls) {
      obj.tool_calls.forEach(tc => {
        const args = tc.args || {};
        const target = args.TargetFile || args.targetFile || args.AbsolutePath || '';
        outStream.write(`Step ${obj.step_index}: Source=${obj.source}, Type=${obj.type}, Tool=${tc.name}, Target=${target}\n`);
      });
    }
  } catch (e) {}
});

rl.on('close', () => {
  console.log('Finished writing to all_tool_calls.txt');
});
